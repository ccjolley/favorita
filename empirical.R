...
augment <- function(train,test) {
  ...
  # For now, if test set contains something that wasn't present in the training
  # set, just assign it the whole-dataset values
  ...
  test <- test %>% 
    ...
    #left_join(by_itemstore,by=c('item_nbr','store_nbr')) %>%
    mutate(test=1,
           date_mean = ifelse(is.na(date_mean),train_mean,date_mean),
